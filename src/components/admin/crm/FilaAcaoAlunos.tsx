...
import { ChevronDown, ChevronRight, MessageCircle, Search, UserCog } from "lucide-react";
...
const PEND: { key: PendKey; label: string; test: (r: OverviewRow) => boolean }[] = [
  ...
  { key: "treino", label: "Treino vencido", test: r => !!r.training_overdue },
  { key: "avaliacao", label: "Avaliação vencida", test: r => !!r.assessment_overdue },
  { key: "ocorrencia", label: "Ocorrências", test: r => (r.open_occurrences || 0) > 0 },
  { key: "experimental", label: "Experimentais", test: r => r.visit_type === "experimental" },
  { key: "sem_responsavel", label: "Sem responsável", test: r => !(r as any).crm_owner_id },
];
...
export default function FilaAcaoAlunos() {
  ...
  return (
    <div className="rounded-xl border border-border bg-card">
      <button onClick={() => setOpen(o => !o)}
        ...
        <div>
          <h2 className="font-barlow font-bold text-lg">Fila de ação de alunos (retenção)</h2>
          <p className="text-xs font-dm text-muted-foreground">
            Pendências por aluno: inadimplência, renovação, frequência, treino, avaliação e ocorrências.
          </p>
        </div>
        <span className="flex items-center gap-2 text-sm font-dm text-muted-foreground shrink-0">
          {loading ? "…" : `${queue.length} pendências`}
          ...
          <div className="flex flex-col md:flex-row gap-3 md:items-center">
            <div className="relative flex-1 md:max-w-xs">
              ...
              <Input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Buscar por nome, telefone, e-mail ou código..." className="pl-9 h-9 font-dm" />
            </div>
      ...
      {detail && <Perfil360 client={detail} onClose={() => setDetail(null)} onSaved={() => { reload(); }} />}
    </div>
  );
}
